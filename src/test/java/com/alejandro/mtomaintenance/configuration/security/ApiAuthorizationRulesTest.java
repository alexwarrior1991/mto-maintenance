...
@Import({SecurityConfiguration.class,
        ...
        ApiAuthorizationRulesTest.ProbeSupervisedController.class})
@TestPropertySource(properties = {
        "app.security.client-id=mto-maintenance-api",
...
class ApiAuthorizationRulesTest {
    ...
    private static final String SUPERVISED = SecurityConfiguration.API + "/probe-orders/cancel";
    ...
    class WithReadRole {
        ...
        void readSucceeds() throws Exception {
            mockMvc.perform(get(PROBES + "/1").with(role(SecurityRoles.MAINTENANCE_READ))).andExpect(status().isOk());
        }
        ...
        void headIsGovernedByTheReadRuleAndNotByPlainAuthentication() throws Exception {
            mockMvc.perform(head(PROBES + "/1").with(role(SecurityRoles.MAINTENANCE_READ))).andExpect(status().isOk());
        ...
        void readingDoesNotGrantWriting() throws Exception {
            mockMvc.perform(post(PROBES).with(role(SecurityRoles.MAINTENANCE_READ)))
                    .andExpect(status().isForbidden())
                    .andExpect(jsonPath("$.errorCode").value("AUTH-403"));
            mockMvc.perform(put(PROBES + "/1").with(role(SecurityRoles.MAINTENANCE_READ))).andExpect(status().isForbidden());
            mockMvc.perform(patch(PROBES + "/1").with(role(SecurityRoles.MAINTENANCE_READ))).andExpect(status().isForbidden());
            mockMvc.perform(delete(PROBES + "/1").with(role(SecurityRoles.MAINTENANCE_READ))).andExpect(status().isForbidden());
        }
    }
    ...
    class WithWriteRole {
        ...
        void writingSucceeds() throws Exception {
            mockMvc.perform(post(PROBES).with(role(SecurityRoles.MAINTENANCE_WRITE))).andExpect(status().isOk());
            mockMvc.perform(put(PROBES + "/1").with(role(SecurityRoles.MAINTENANCE_WRITE))).andExpect(status().isOk());
            mockMvc.perform(patch(PROBES + "/1").with(role(SecurityRoles.MAINTENANCE_WRITE))).andExpect(status().isOk());
        }

        @Test
        void writingDoesNotGrantReadingOrDeleting() throws Exception {
            mockMvc.perform(get(PROBES + "/1").with(role(SecurityRoles.MAINTENANCE_WRITE))).andExpect(status().isForbidden());
            mockMvc.perform(delete(PROBES + "/1").with(role(SecurityRoles.MAINTENANCE_WRITE))).andExpect(status().isForbidden());
        }
        ...
        void writingDoesNotGrantAdjusting() throws Exception {
            mockMvc.perform(post(SUPERVISED).with(role(SecurityRoles.MAINTENANCE_WRITE)))
                    .andExpect(status().isForbidden())
                    .andExpect(jsonPath("$.errorCode").value("AUTH-403"));
        }
    }
    ...
    class WithDeleteAndAdjustRoles {
        ...
        void deletingSucceedsOnlyWithItsOwnRole() throws Exception {
            mockMvc.perform(delete(PROBES + "/1").with(role(SecurityRoles.MAINTENANCE_DELETE))).andExpect(status().isNoContent());
        }
        ...
        void adjustingNeedsBothTheWriteAndTheAdjustRole() throws Exception {
            mockMvc.perform(post(SUPERVISED).with(role(SecurityRoles.MAINTENANCE_SUPERVISE))).andExpect(status().isForbidden());
            mockMvc.perform(post(SUPERVISED).with(role(SecurityRoles.MAINTENANCE_WRITE, SecurityRoles.MAINTENANCE_SUPERVISE)))
    ...
    /** Réplica de la guarda que lleva el endpoint real de cancelacion en {@code MaintenanceOrderController}. */
    @RestController
    @RequestMapping(SecurityConfiguration.API + "/probe-orders")
    static class ProbeSupervisedController {
